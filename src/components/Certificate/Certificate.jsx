  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            setCertificateId(name.substring(0,3)+'_'+orgname.substring(0,3)+'_COU_'+cnt.toString());
        } catch (error) {
    ...
    try {
      ...
      if(!resData.isDuplicate)
      {
        ...
        const res = await fetch("http://localhost:3001/addcertno", {
            ...
            body: JSON.stringify({name:orgname,certificateType:"COU",certificateId:certificateId}) 
        });
      }
