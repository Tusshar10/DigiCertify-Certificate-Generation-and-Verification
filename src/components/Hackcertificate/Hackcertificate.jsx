...
import styles from '../Hackgenerate/HackcertificateGenerator.module.scss'
  ...
  useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:3001/getcertno/${orgname}/HACK`); // Replace orgname and type with actual values
                if (!response.ok) {
                ...
                setCertificateId(name.substring(0,3)+'_'+orgname.substring(0,3)+'_HACK_'+cnt.toString());
            } catch (error) {
    ...
    try {
      ...
      if(!resData.isDuplicate)
      {
        ...
        const res = await fetch("http://localhost:3001/addcertno", {
            ...
            body: JSON.stringify({name:orgname,certificateType:"HACK",certificateId:certificateId}) 
        });
      }
    ...
    <>
      <div ref={certificateRef} className={styles.certificateWrapper}>
        ...
        <div className="mt-3"style={{ textAlign: "right" }}>Certificate Id: {certificateId}</div>
          ...
          {position==='Participant'? <span className={styles.smallText}>of {person_details} as Team {teamname} has participated in the </span>:<span className={styles.smallText}>of {person_details} as Team {teamname} has achieved <b>{position}</b> position in the </span>}
          ...
          <div className={styles.signatureBlock}>
            ...
            <img className={styles.signatureImage} src={hacksignature.preview} alt='' />

            <span className={styles.horizontalBar} />

            <span className={styles.smallText}>{hacksignatureDetails}</span>
            </div>
          </div>
        </div>
